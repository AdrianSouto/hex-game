...
class Player:
    ...
    def play(self, board: HexBoard) -> tuple:
        if self.is_ai:
            ...
            _, play = self.minimax(board, MAX_DEPTH, True, float('-inf'), float('inf'))
    ...
    def minimax(self, board: HexBoard, depth: int, maximizing_player: bool, alpha: float, beta: float) -> Tuple[
        ...
        for move in posible_moves:
            ...
            if cloned_board.check_connection(actual_player):
                return (100 if maximizing_player else -100), move

            # Recursive call
            score, _ = self.minimax(cloned_board, depth - 1, not maximizing_player, alpha, beta)
